...
describe('runLinters', () => {
  it('runs all given linters in parallel and merges their reports', async () => {
    const { report: merged, skipped } = await runLinters({ cwd: '/repo', linters: [oxcD, biomeD] }, (d) =>
      Promise.resolve(report(d.linter)),
    );
  ...
  it('throws when every detected linter fails', async () => {
    await expect(
      runLinters({ cwd: '/repo', linters: [oxcD, biomeD] }, () => Promise.reject(new Error('boom'))),
