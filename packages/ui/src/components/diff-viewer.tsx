/** biome-ignore-all lint/suspicious/noArrayIndexKey: diff lines/files are positional; index IS the identity */
'use client';

/*
 * DiffViewer — vendored from the assistant-ui community registry:
 * https://r.assistant-ui.com/diff-viewer.json
 *
 * Adaptations from upstream:
 *  - `@/lib/utils` rewritten to the package-relative path
 *  - Dropped `@assistant-ui/react-markdown` import + the `code` prop alias of
 *    `patch` (the markdown dep is large and only contributed a type)
 *  - shadcn semantic tokens (bg-background, text-muted-foreground, bg-muted,
 *    border) mapped to the project's zinc palette
 *
 * Used by lintscope to render before/after for ESLint autofixes inside
 * `<DiagnosticCard />`.
 */
      ...
      <span className="flex-1">
        {oldName && newName && oldName !== newName ? (
          <>
            ...
            {' → '}
            <span className="text-emerald-600 dark:text-emerald-400">{newName}</span>
          </>
        ) : (
          displayName
        )}
...
export type DiffViewerProps = VariantProps<typeof diffViewerVariants> & {
  /** Unified-diff patch string (takes precedence over oldFile/newFile). */
  patch?: string;
...
function DiffViewer({
  ...
  const parsedFiles = useMemo(() => {
    ...
    if (oldFile && newFile) {
      ...
      return [
        {
          ...(oldFile.name ? { oldName: oldFile.name } : {}),
          ...(newFile.name ? { newName: newFile.name } : {}),
          lines,
          additions,
          deletions,
        },
